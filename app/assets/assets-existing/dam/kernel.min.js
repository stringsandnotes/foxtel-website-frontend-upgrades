...
v.push(b[r]);
t=o
}}}}}if(v.length>0){importantUpdateHideShowFunc(false);
foxtelTeaserHideShowFunc(false,g);
var u=v[0];
if(CQ_Analytics.StrategyMgr){var s=CQ_Analytics.StrategyMgr.choose(f,v);
if(s!=null){u=s
}}if(!k||k.path!=u.path){k=u;
...
q.teaserToolTip.remove();
q.teaserToolTip=null
}}else{foxtelTeaserHideShowFunc(true,g)
}CQ_Analytics.Utils.clearElement(g);
k=null
}};
j.call();
...
}}}var importantUpdateHideShowFunc=function(a){var b=$(".important-update");
if(b&&b.length>0){if(a){b.hide(0)
}else{b.show(0)
}}};
var foxtelTeaserHideShowFunc=function(a,c){var b=$(".important-update");
var d=$("#"+c);
if(d&&d.length>0){var e=d.parent();
if(a){e.hide(0);
$(".important-update-after-js",e).remove()
}else{e.show(0)
}$(".important-update-after-js:first").hide(0)
}};
...
if(!CQ_Analytics.FoxtelCustomerMgr){CQ_Analytics.FoxtelCustomerMgr=CQ_Analytics.JSONStore.registerNewInstance("foxtelcustomer");
this.data={accountType:"",rateClassDefault:"",bundleId:"",tierId:"",statusId:"",statusReasonId:"",collectionStatus:"",accountStatus:"",orderList:"",servicePostalCode:"",serviceOrderTypeId:"",workOrderList:""};
CQ_Analytics.FoxtelCustomerMgr.loadData=function(a){if(typeof a!=="object"){this.data=JSON.parse(a)
}else{this.data=a
}};
CQ_Analytics.FoxtelCustomerMgr.addListener("update",function(b,c){if(c&&typeof c==="object"){this.loadData(c)
}else{var a={accountType:"",rateClassDefault:"",bundleId:"",tierId:"",statusId:"",statusReasonId:"",collectionStatus:"",accountStatus:"",orderList:"",servicePostalCode:"",serviceOrderTypeId:"",workOrderList:""};
this.loadData(a)
}});
CQ_Analytics.FoxtelCustomerMgr.getValue=function(a){if(CQ_Analytics.FoxtelCustomerMgr.data){if(CQ_Analytics.FoxtelCustomerMgr.data[a]){return CQ_Analytics.FoxtelCustomerMgr.data[a].value
}}return""
};
CQ_Analytics.FoxtelCustomerMgr.setTraitValue=function(a,c){var b=CQ_Analytics.FoxtelCustomerMgr.data;
if(b){for(property in b){if(property===a){b[property]=c
}}}return b
}
};